...
def systemList(request):
    try:
        ...
        p = ApiUtil.requestGet(url, FuncCode.systemList.value, data)
        systems = p['lists']
    ...
    return render(request, "gui_app/system/systemDetail.html", {'system': '', 'baseImage':'', 'message':ex})
...
def systemCreate(request):
    try:
        if request.method == "GET":
            ...
            envs = ApiUtil.requestGet(url, FuncCode.systemCreate.value, p)

            p.update({'envs': envs['lists']})
            print(p)

            return render(request, Html.systemCreate, {'system': p,'message': ''})
        else:
...
def systemEdit(request, id):
    try:
        if request.method == "GET":
            ...
            envs = ApiUtil.requestGet(url2, FuncCode.systemEdit.value, data)

            return render(request, Html.systemEdit, {'system': p, 'envs': envs['lists'],'message': ''})
        else:
            # -- Get a value from a form
            msg = ''
            p = request.POST
